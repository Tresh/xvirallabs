...
export function AnalyzeTab() {
  ...
  const { remaining, isUnlimited, hasReachedLimit, dailyLimit, decrementLocal } = useDailyUsage();
  ...
  const handleSave = async () => {
    ...
    try {
      if (selectedMode === 3) {
        ...
        if (!saveError) {
          setHasSaved(true);
          toast({ title: "Pattern saved!", description: "View it in your Analyses tab." });
        }
        ...
        if (!saveError) {
          setHasSaved(true);
          toast({ title: "Ideas saved!", description: "View them in your Analyses tab." });
        }
      } else {
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-muted-foreground mt-1 text-sm">Paste any tweet and let AI reverse-engineer its virality.</p>
      </div>

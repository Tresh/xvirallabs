...
export default function Memory() {
  ...
  // Load data from profile/brandVoice
  useEffect(() => {
    if (profile) {
      setDisplayName((profile as any).display_name || "");
      setTwitterHandle(profile.twitter_handle || "");
      setSkills((profile as any).skills || []);
      setContentStrategy((profile as any).content_strategy || "");
      setCustomSystemPrompt((profile as any).custom_system_prompt || "");
  ...
  }, [user, displayName, twitterHandle, skills, contentStrategy, customSystemPrompt, brandTone, primaryNiche, writingTraits, wordsToAvoid, signaturePhrases, preferredHooks, refreshProfile]);

  // Debounced auto-save
  const triggerSave = useCallback(() => {
    if (debounceRef.current) clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(saveAll, 1500);
  }, [saveAll]);

  // Trigger save on field changes (skip initial load)
  ...
  useEffect(() => {
    ...
    triggerSave();
  }, [displayName, twitterHandle, skills, contentStrategy, customSystemPrompt, brandTone, primaryNiche, writingTraits, wordsToAvoid, signaturePhrases, preferredHooks]);
      ...
      <main className="max-w-2xl mx-auto p-4 md:p-8 space-y-6">
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">
              Describe what you're building, your audience, and how you want to grow. This shapes all generated content.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Textarea
              ...
              placeholder="e.g., I'm building a personal brand around AI productivity. My audience is solopreneurs and indie hackers. I want to grow by sharing actionable tips and building in public."
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">
              Custom instructions for the AI. This is injected as a system-level directive — the AI will follow these rules but never repeat them.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Textarea
              ...
              placeholder="e.g., Always write in first person. Never use emojis. Keep sentences under 15 words. Don't use 'leverage' or 'utilize'. Sound like a builder, not a guru."
